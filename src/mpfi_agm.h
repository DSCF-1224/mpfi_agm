...
static void mpfi_agm(mpfi_ptr res, mpfi_srcptr op1, mpfi_srcptr op2)
{
    ...
    mpfi_t intersection_ag, res_a, res_g, tmp_a, tmp_g, union_ag;

    mpfi_init_set(intersection_ag, res);

    mpfi_init2( res_a    , mpfi_prec ); mpfi_set(res_a, op1);
    mpfi_init2( res_g    , mpfi_prec ); mpfi_set(res_g, op2);
    ...
    {
        mpfi_agm_step_tmp(res_a, res_g, tmp_a, tmp_g, res_a, res_g);

        mpfi_intersect(intersection_ag, res_a, res_g);

        if ( mpfi_is_empty(intersection_ag) ) continue;
